...
class AntraxIRVisitor(ParseTreeVisitor):
    ...
    def visitExpr(self, ctx: AntraxParser.ExprContext):
        """
        Genera código LLVM IR para expresiones.
        """
        ...
        if ctx.STRING():
            ...
            # Crea una cadena global en el módulo LLVM
            global_str = ir.GlobalVariable(
                self.__module,
                ir.ArrayType(
                    ir.IntType(8), len(string_val) + 1
            ...
            global_str.initializer = ir.Constant(
                ir.ArrayType(ir.IntType(8), len(string_val) + 1),
                bytearray(
                    string_val.encode("utf-8") + b"\x00"
                ),  # Asegura que termine en '\0'
            )
            # Obtén un puntero al primer carácter (i8*)
            str_ptr = self.__builder.gep(
                global_str,
                [ir.Constant(ir.IntType(32), 0), ir.Constant(ir.IntType(32), 0)],
                name="str_ptr",
        ...
        if ctx.getChildCount() == 3:
            ...
            if op == "+":
                return self.__builder.add(left, right, name="addtmp")
            if op == "-":
                return self.__builder.sub(left, right, name="subtmp")
            if op == "*":
                return self.__builder.mul(left, right, name="multmp")
            if op == "/":
                return self.__builder.sdiv(left, right, name="divtmp")

            # Comparaciones Booleanas
            if op == "<":
                return self.__builder.icmp_signed("<", left, right, name="cmplttmp")
            if op == ">":
                return self.__builder.icmp_signed(">", left, right, name="cmpgttmp")
            if op == "<=":
                return self.__builder.icmp_signed("<=", left, right, name="cmpletmp")
            if op == ">=":
                return self.__builder.icmp_signed(">=", left, right, name="cmpgetmp")
            if op == "==":
                return self.__builder.icmp_signed("==", left, right, name="cmpeqtmp")
            if op == "!=":
                return self.__builder.icmp_signed("!=", left, right, name="cmpnetmp")
